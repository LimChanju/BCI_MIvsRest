...
for f in files:
    ...
    if "769" in event_dict: left, right = event_dict["769"], event_dict["770"]
    elif 769 in event_dict: left, right = event_dict[769], event_dict[770]
    else:
    ...
    rest_ep = mne.Epochs(raw, ev, event_id, tmin=-4.0, tmax=0.0,
                         baseline=None, preload=True, verbose=False)
    mi_ep   = mne.Epochs(raw, ev, event_id, tmin= 0.0, tmax=4.0,
                         baseline=None, preload=True, verbose=False)
    X_subj = np.concatenate([rest_ep.get_data(), mi_ep.get_data()], axis=0)
    y_subj = np.concatenate([np.zeros(len(rest_ep)), np.ones(len(mi_ep))])
...
def neuro_mel_encc_for_subject(trials, fs=250, n_fft=256, hop=128, n_ceps=16):
    iaf=estimate_iaf(trials.mean(0)[0],fs)
    fb,centers=neuro_filterbank(fs,n_fft,iaf)
    NM_list,ENCC_list=[],[]
    for sig in trials[:,0]:
        P=stft_power(sig,n_fft,hop)
        NM=fb@P; NM=np.log(np.clip(NM,1e-8,None))
...
print("\n🎧 Converting → Neuro-Mel + ENCC ...")
NM_cat, EN_cat = [], []
for s in np.unique(subj_ids):
    mask=subj_ids==s
    NM,ENCC,cent=neuro_mel_encc_for_subject(X[mask])
...
X_feat=np.stack([NM_all,EN_all],axis=1)
...
class NeuroMel_ViT_DS(nn.Module):
    def __init__(self,F=40,T=64,num_classes=2,patch=4,dim=64,depth=3,heads=4,mlp_dim=128):
        ...
        enc=TransformerEncoderLayer(d_model=dim,nhead=heads,dim_feedforward=mlp_dim,
...
for subj in subjects:
    ...
    plt.title(f"Subject {subj} - 10×10 CV (NeuroMel ViT)")
...
plt.title("Subject-wise Accuracy (Neuro-Mel ViT 10×10 CV)")
