...
for f in files:
    ...
    X_rest = rest_epochs.get_data()   # (n_trials, n_channels, times)
...
# 2. 데이터 합치기
# --------------------------
X = np.concatenate(X_all, axis=0)  # (trials, n_channels, times)
y = np.concatenate(y_all, axis=0)
print("\nFinal data shape:", X.shape)  # (N, 3, T)
print("Final labels:", np.unique(y, return_counts=True))

# Torch tensor 변환
X = torch.tensor(X, dtype=torch.float32)  # (trials, 3, times)
...
class EEGNet(nn.Module):
    def __init__(self, n_channels=3, n_times=500, n_classes=2, F1=8, D=2, F2=16, kernel_length=64, dropout=0.25):
    ...
    def forward(self, x):
        # x: (batch, channels, times) → (batch, 1, channels, times)
        x = x.unsqueeze(1)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.depthwise(x)
...
# 4. 모델 초기화
# --------------------------
model = EEGNet(n_channels=3, n_times=X.shape[2], n_classes=2)
...
plt.tight_layout()
plt.savefig("./CNN_result/EEGNet_C3C4Cz.png", dpi=300)
plt.show()
