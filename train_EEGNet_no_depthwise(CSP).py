...
# --------------------------
# 3. 단채널 EEGNet 정의 (Depthwise 없음)
# --------------------------
class EEGNet1Ch(nn.Module):
    def __init__(self, n_times=500, n_classes=2, F1=8, F2=16, kernel_length=64, dropout=0.25):
        super(EEGNet1Ch, self).__init__()
        self.n_times = n_times

        # 1. Temporal Convolution (주파수 특징 추출)
        ...
        # 2. Separable Convolution (시계열 패턴 요약)
        self.separable_conv = nn.Sequential(
            nn.Conv2d(F1, F1, (1, 16), padding=(0, 8), groups=F1, bias=False),  # depthwise temporal
            nn.Conv2d(F1, F2, (1, 1), bias=False),  # pointwise
            nn.BatchNorm2d(F2),
            nn.ELU(),
            nn.AvgPool2d((1, 8)),
            nn.Dropout(dropout)
    ...
    def _get_out_size(self):
        with torch.no_grad():
            x = torch.zeros(1, 1, 1, self.n_times)  # (batch, 1, channels=1, times)
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.elu(x)
            x = self.pool1(x)
    ...
    def forward(self, x):
        # 입력: (batch, 1, times) → (batch, 1, 1, times)
        x = x.unsqueeze(2)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.elu(x)
...
# 4. 모델 초기화
# --------------------------
model = EEGNet1Ch(n_times=X.shape[2], n_classes=2)
...
plt.tight_layout()
plt.savefig("./CNN_result/EEGNet1Ch_C3.png", dpi=300)
plt.show()
