...
class EEGNet1Ch(nn.Module):
    def __init__(self, n_times=1000, n_classes=2, F1=8, F2=16, kernel_length=64, dropout=0.25):
        ...
        self.separable_conv = nn.Sequential(
            nn.Conv2d(F1, F1, (1, 16), padding=(0, 8), groups=F1, bias=False),
...
# --------------------------
# 4. 10×10 subject-dependent CV
# --------------------------
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
unique_subjects = np.unique(subj_ids)
save_dir = "./EEGNet_result_no_depthwise_10x10"
...
for subj in unique_subjects:
    ...
    print(f" → Subject {subj} mean acc = {mean_acc:.3f} ± {std_acc:.3f}")
...
# --------------------------
# 6. 모든 subject 결과 막대그래프 시각화
# --------------------------
subjects = [f"Subj{s}" for s in unique_subjects]
plt.figure(figsize=(8, 5))
plt.bar(subjects, subject_results, color='skyblue', edgecolor='k')
...
plt.title("Subject-wise Mean Accuracy (10×10 CV)")
plt.ylabel("Accuracy")
plt.ylim(0, 1)
